<template>
	<div class="bookcity">
		...
		<div class="nav">
			<van-search v-model="value" placeholder="请输入搜索关键词" @focus="goSearch" shape="round" />
		</div>
		<div class="swipe-box">
		...
		<div class="grild-box">
			<van-grid>
				<van-grid-item icon="qr-invalid" @click="goClass" text="分类" />
				<van-grid-item icon="bar-chart-o" @click="goRank" text="排行" />
				<van-grid-item icon="orders-o" text="书单" />
			...
			<van-tabs v-model="active" scrollspy sticky>
				<van-tab v-for="(item,index) in bookCityData" :key="index" :title="item.title">
					...
					<ul class="rank-list clearfix" v-show="isActive">
						<li class="home-card" @click="goDetail(item1._id)" v-for="item1 in item.bookArr" :key="item1._id">
							<div class="thumb">
							...
							<div class="book-detail">
								...
								<div class="span-ap">
									<span class="span-author">
										<van-icon name="manager" />{{item1.author}} | {{item1.minorCate}}</span>
									<span class="span-person">14.19万人气</span>
								</div>
							</div>
						</li>
					</ul>
					<ul class="rank-list clearfix" v-show="!isActive">
						<li class="home-card" @click="goDetail(item1._id)" v-for="item1 in item.bookgirlArr" :key="item1._id">
							<div class="thumb">
								<img :src="item1.cover" alt="" />
								<h5>{{item1.title}}</h5>
							</div>
